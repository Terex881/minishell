...
/*   Updated: 2024/05/08 18:46:18 by cmasnaou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_expand_var(t_list **list, char *str)
{
	...
	{
		tmp->value = ft_strjoin(ft_itoa(exit_status(0, 0)), ft_substr(tmp->value, 2, ft_strlen(tmp->value) - 2));
	}
	if (str && !ft_strncmp(str, "$?", 2) && tmp->type == D_Q)
	{
		tmp->value = ft_strjoin(ft_substr(tmp->value, 0, str - tmp->value), ft_itoa(exit_status(0, 0)));
